...
@click.option('--overwrite/--no-overwrite', default=False, help='Overwrite previously extracted files (if any) from original file')
...
def main(input_path, delete, slurm, cores, memory, wall_time, partition, overwrite):
    ...
    if slurm:
        ...
        for mkvfile in mkvfiles:
            os.system('sbatch -p {} -t {} --mem {} -c {} --wrap """mkv-extract {} {} {}"""'.format(
                partition, wall_time, memory, cores, mkvfile, '--delete' if delete else '', '--overwrite' if overwrite else ''))
            
    else:
        for mkvfile in tqdm.tqdm(mkvfiles):
            compress_mkv(mkvfile, verbose=True, delete=delete, overwrite=overwrite)
